"""
Crawler für deutsche-evergabe.de

ANPASSUNG ERFORDERLICH:
1. Öffne https://www.deutsche-evergabe.de im Browser
...
class DeutscheEvergabeCrawler(BaseCrawler):
    """Crawler für deutsche-evergabe.de"""
    ...
    LOGIN_URL = "/login"
    USERNAME_SELECTOR = 'input[name="username"], input[name="email"], input[type="email"], #login-email'
    PASSWORD_SELECTOR = 'input[name="password"], input[type="password"], #login-password'
    LOGIN_BUTTON_SELECTOR = 'button[type="submit"], .login-submit, .btn-login'
    LOGIN_SUCCESS_INDICATOR = 'logout, abmelden, mein konto, dashboard'
    
    SEARCH_URL = "/ausschreibungen"
    SEARCH_INPUT_SELECTOR = 'input[name="q"], input.search-field, input[type="search"]'
    TENDER_LINK_SELECTOR = 'a[href*="/ausschreibung/"], a[href*="/tender/"], .tender-item a'
    
    TITLE_SELECTOR = 'h1, .tender-title, .ausschreibung-title'
    AUTHORITY_SELECTOR = '.auftraggeber, .contracting-entity'
    DESCRIPTION_SELECTOR = '.beschreibung, .tender-description, .leistung'
    DEADLINE_SELECTOR = '.abgabetermin, .deadline, .frist'
    BUDGET_SELECTOR = '.auftragswert, .estimated-value'
